...
## Release Notes
> ### v1.0.1 (02/05/2021)
> - Initial web application infrastructure has been created to allow access to TipTracker via [URL](https://tiptracker.vercel.app) or locally.
...
>
> ### v1.0.2 (02/26/2021)
> - Login and Registration pages have been added. Visitors can now register an account and login to the application.
...
>  - Data from the Work Summary Screen can be exported to a `csv` file.
>  - The interface for the Upcoming Work Schedule has been created.
>  #### Bug Fixes
>  - The layout of the Work Calendar Screen has been reworked to be compatible with smaller screen sizes.
>  - Unused icons from the Navigation bar have been removed.
>
> <hr/>
>
>  ### v1.0.3 (03/12/2021)
>  - An initial prototype interface for the Analytics Screen has been created.
>  - The work summary for the selected date range can now be viewed on the Work Summary Screen.
>  - Data pipeline has been added using database hooks to process shift data for the Machine Learning model.
>  #### Bug Fixes
...
>
>  ### v1.0.4 (04/02/2021)
>  - The interface for the Analytics Screen has been updated with improved keys and color schemes.
>  - The interface for the User Profile has been created.
>  - Microservice for the Machine Learning model has been integrated with the app.
>  #### Bug Fixes
...
>   - The data processing pipeline has been updated to split the shifts accurately.
>
> <hr/>
>
>   ### v1.1.0 (04/23/2021)
...
>  #### Bug Fixes
>   - The correct week on the Past/Future Analytics Page is shown when the first day of the week is selected on the previous screen.
>   #### Bugs Discovered
>   - When a user updates their email, the JWT will still include the old email. So all additional requests made to the backend will become unauthenticated.
...
Docker allows to easily use Docker images to set up the database required for this application. <br />
...
This installer contains all detailed instructions to set up the database. It is set to start the database on Startup by default.
...
This is a visual database tool that allows us to easily access the MySQL database. You can download it directly from [here](https://dev.mysql.com/downloads/workbench/). It is availabe for most Operating Systems.
...
**NOTE:** This is not required if you are using downloading the project as a ZIP file instead of cloning the repository.

</li>
</ol>

If you want to use the Future Analytics feature, see additional pre-requisites [here](https://github.com/JIA-0302/Analytics#pre-prequisites).
...
If you want to use the Future Analytics feature, see instructions [here](https://github.com/JIA-0302/Analytics#installing-dependent-libraries) to install additional dependent libraries.
...
1. Setup environment variables
    ...
    Note, these variables can be set directly. However, by using `.env.local`, these variables are only available during the runtime of the application and are easier to manage.

    In the production environment, make sure to update `http://localhost:*` with the URL of the deployed application. 
...
2. Setup the database schema
    - **NOTE:** This step can be run only once. If you already set up the schema previously, no action is needed.
    ...
    - Go to `File -> Run SQL Script` to run the following scripts:
      ...
      - `/scripts/database/db-seed.sql`: This will populate the database with required initial data.

3. Setup the Machine Learning Microservice
    - To prevent having a Monolithic repo, all source code pertaining to the ML model is in [Analytics submodule](https://github.com/JIA-0302/Analytics).
...
1. `Error: connect ECONNREFUSED 127.0.0.1:3306`
    ...
    This typically means the MySQL database is not running. See the installation guide [here](#installation-guide) to start the database.

    Also ensure the [environment variables](#build-instructions) for the MySQL database have been set properly.

2. `module not found. Error: Cannot resolve module ...`

    This means one of the dependent libraries was not installed. To fix this issue run the following command:
...
2. Setup environment variables.
    * Create `.env.local` file to store all necessary environment variables for the development environment.
    * Copy contents from `.env.example` to `.env.local.` It has default environment variables set.
    * Replace all occurrences  of `xxxxxxx` with values specific to your configuration.
...
To enable the Analytics functionality, we need to add a [different repo](https://github.com/JIA-0302/Analytics). This is already included as a submodule in this repo.
...
3. For more information on running the service, view the instructions [here](https://github.com/JIA-0302/Analytics#setup).
...
### Committing Changes
Create a Pull request against the `main` branch to make any changes. You must request a review and you cannot merge it to `main` until it is approved.

After it is approved, perform `Squash and merge`. This combines all the Pull Request commits into a single commit on the `main` branch. This helps us have a clean commit log in the `main` branch.
