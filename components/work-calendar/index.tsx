...
const WorkCalendar: React.FunctionComponent = () => {
  ...
  const cells = () => {
    ...
    if (shiftIds.hourlyShiftDetails && shiftIds.nonHourlyShiftDetails) {
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      joinedShiftDates = shiftIds.hourlyShiftDetails.concat(
        shiftIds.nonHourlyShiftDetails
      );
      ...
      for (let i = 0; i < 7; i++) {
        ...
        if (isSameMonth(day, monthStart)) {
          ...
          if (shiftIds.hourlyShiftDetails && shiftIds.nonHourlyShiftDetails) {
            ...
            // @ts-ignore
            if (
              joinedShiftDates[traverseIndex] &&
              cloneDay.toISOString().substring(0, 10) ===
                joinedShiftDates[traverseIndex].shift_date.substring(0, 10)
            ) {
        ...
        days.push(
          ...
          >
            ...
            {foundShift ? <span className={styles.check}>&#9989;</span> : ""}
          </div>
        );
        day = addDays(day, 1);
