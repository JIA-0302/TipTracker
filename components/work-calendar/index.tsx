...
const WorkCalendar: React.FunctionComponent = () => {
    ...
    const [mockEditData, setMockEditData] = useState({ "earnings": 110.50, "cashTips": 230, "ccTips" : 55, "active":true});
    ...
    const createShiftModal = (calendarDate) => {
        return (
                ...
                showAddModal || !showEditModal ? <ShiftMainAddModal date={calendarDate} show={showAddModal} onHide={setShowAddModal} />
                    : <ShiftEditModal date={calendarDate} show={showEditModal} onHide={setShowEditModal} data={{mData:mockEditData, updatedata: setMockEditData}} />
            ...
            for (let i = 0; i < 7; i++) {
                ...
                days.push(
                    <div
                        ...
                        onClick={() => cloneDay.toDateString() === "Sun Feb 14 2021" && mockEditData.active ? onDateClick(toDate(cloneDay), "edit") : onDateClick(toDate(cloneDay), "add")}
                    >
                        <span className={styles.number}>{formattedDate}</span>
                        {cloneDay.toDateString() === "Sun Feb 14 2021" && mockEditData.active ?
                        < span className={cloneDay.toDateString() === "Sun Feb 14 2021"  ? styles.check : ''
                        }>&#9989;</span>
                            : ''
                        }
                    </div>
